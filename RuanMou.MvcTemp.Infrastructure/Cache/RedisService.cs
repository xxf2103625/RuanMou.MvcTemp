...
{
    /// <summary>
    /// CacheService，是redis操作类，继承自IDisposable接口，主要用于释放内存
    /// </summary>
    public class RedisService : IDisposable
    {
        ...
        public RedisService(int dbNum = 0)
                : this(dbNum, null)
        {
        }

        public RedisService(int dbNum, string readWriteHosts)
        {
            DbNum = dbNum;
            _conn =
                string.IsNullOrWhiteSpace(readWriteHosts) ?
                RedisConfig.Instance :
                RedisConfig.GetConnectionMultiplexer(readWriteHosts);
        }
